...
def evaluate(data, model, onset_threshold=0.5, frame_threshold=0.5, save_path=None):
    ...
    for label in data:
        pred, loss = model.run_on_batch(label['audio'], label['label'])

        metrics['loss/eval'].append(loss.item())

        pred_data = torch.softmax(pred, dim=-1)
        ...
        t_est, f_est = notes_to_frames(p_est, i_est, pred_data.shape)
        ...
        if save_path is not None:
            ...
            save_pianoroll(pred_path, pred_onsets, pred_frames)
